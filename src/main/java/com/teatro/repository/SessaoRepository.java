 ...
 * 
 * Fornece métodos para: - Busca de sessões por data e horário - Lógica de horários dinâmicos -
...
public interface SessaoRepository extends JpaRepository<Sessao, Long> {
        ...
        */
       List<Sessao> findByAtivaTrueAndEventoId(Long eventoId);
        ...
        */
       List<Sessao> findByAtivaTrueAndDataSessao(LocalDate dataSessao);
        ...
        */
       List<Sessao> findByAtivaTrueAndEventoIdAndDataSessao(Long eventoId, LocalDate dataSessao);

       /**
        * Busca sessões por evento, data e tipo
        */
       List<Sessao> findByEventoIdAndDataSessaoAndTipoSessao(Long eventoId, LocalDate dataSessao,
                     TipoSessao tipoSessao);

       /**
        * Busca sessões futuras (data >= hoje)
        */
       @Query("SELECT s FROM Sessao s " + "WHERE s.ativa = true "
                     + "AND s.dataSessao >= CURRENT_DATE " + "ORDER BY s.dataSessao, s.horario")
       List<Sessao> findSessoesFuturas();

       /**
        * Busca sessões futuras por evento
        */
       @Query("SELECT s FROM Sessao s " + "WHERE s.ativa = true " + "AND s.evento.id = :eventoId "
                     + "AND s.dataSessao >= CURRENT_DATE " + "ORDER BY s.dataSessao, s.horario")
        ...
        */
       boolean existsByEventoIdAndDataSessaoAndHorario(Long eventoId, LocalDate dataSessao,
                     LocalTime horario);

       /**
        * Busca sessões com maior ocupação
        */
       @Query("SELECT s, COUNT(i) as totalIngressos " + "FROM Sessao s "
                     + "LEFT JOIN s.ingressos i " + "WHERE s.ativa = true " + "GROUP BY s "
